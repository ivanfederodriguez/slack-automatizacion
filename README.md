...
python main.py reprocess-open-trello --dry-run --only-open-trello-live
...
python main.py reprocess-open-trello --apply --only-open-trello-live
```

Por seguridad, este comando no envía mensajes a Slack ni en `--dry-run` ni en `--apply`. En `--dry-run` solo imprime cambios posibles y distingue `category_changed`, `public_text_changed_only`, `trello_create` y `trello_update`. `--only-open-trello-live` consulta cada card vinculada y usa Trello como fuente de verdad para incluir solo las que siguen abiertas, incluso si el estado local quedó desincronizado. Los filtros por categoría y `--task-id` permiten acotar el lote antes de aplicar cambios. Con `--apply`, actualiza SQLite y Trello para tareas abiertas con cambios relevantes, agrega auditoría interna y deja explícito en el resumen `Slack mensajes enviados: 0`.
